...
| T-CONQ-1 | **Conquest → territory flip** — a real mechanic by which winning combat/mission at a world calls `claimHolding(S,pid)` (and losing calls `releaseHolding`), replacing T-TERR-1's dev-claim placeholder button. Ties battle/mission thread outcomes to `S.holdings`. | 🔥 engine (+ THREAD core) | `open` | — | — | **Depends on T-TERR-1 (holdings + claim seam) + combat threads.** T-TERR-1 ships the pure `claimHolding`/`releaseHolding` seam + a placeholder dev-claim button; THIS task wires the earn path (thread victory → claim). Deferred by design — pick up in a later session. |
| T-TERR-1 | **Territory persistence + per-sector production (THE FLYWHEEL)** — save-state ownership of sectors/worlds so the living-world tick accrues by HOLDINGS, not a flat demo constant | 🔥 engine + canon | `claimed` | opus · sess:b87383b0-7074-4df6-a4dd-093277cd57c6 | 2026-07-20 | **P1 — the "world accumulates" payoff + unblocks G6 production coupling & §4.3 garrison.** (1) add owned-holdings to `S`; (2) drive `WORLD` production per-sector from `planet_types.prod_mult` × holdings × `RIFT.mods('home').prodMult`; (3) surface in the Digest. Extends the shipped `WORLD` tick + storage-adapter seam (real accumulation lands with Stage-2 persistence; structure buildable now). |
