...
| T-CMB-1 | **Combat tag-conditions tick & apply** — instance-based conditions (DoT/Regen/Slowing/Suppressing/…) tick, gate, and expire in the THREAD core | 🔥 engine + tests | `in-progress` | missions · sess:8072f7bf-6c27-4be2-a813-56124ad4cb90 | 2026-07-28 | **Drift-aligned + 3 rulings 2026-07-28** (NL+DoT floors ticks · aftermath freezes conds · DoT kills = full mission/weapon credit; spec addendum + plan drift section committed `e9551eb`). **DESIGN LOCKED 2026-07-25** — spec `docs/superpowers/specs/2026-07-25-combat-conditions-design.md`. Re-scoped: COMBAT TAGS ONLY (the 9 `galaxy.conditions` + `status_effects` are location/sector state → moved to T-STAT-1). Locked: afflicted-side tick anchor, FIFO tick order, hard validate gates, no stacking (refresh-or-upgrade), Suppressing capped at −1 action (tier = pin duration), extensible registry (unknown tags inert-but-displayed). Phase 2 = Shield/Ward/Decoy damage-step trio. Fixes the cond-label/self-target staging bug in passing. |
| T-ECN-1 | **Economy E1 — resource core** — canon `resource_output` (20 types) + `rules.resources`; WORLD tick: location-share production, Food consumption, upkeep (the Tithe), overflow loss; per-holding stockpiles; HQ Ledger + map stock display | canon + 🔥 engine + tests | `open` | — | 2026-07-27 | **DESIGN LOCKED** — spec `docs/superpowers/specs/2026-07-27-economy-resources-sinks-design.md` §7 Phase E1. Self-contained; build first. |
| T-ECN-2 | **Economy E2 — sinks at doors** — door leveling (currency+resources, build timers), gear wear+repair+melt-down, consumable packs, tier-gated door services (Altar blessings ride CONDS) | canon + 🔥 engine + tests | `blocked` | — | 2026-07-27 | Blocked on T-ECN-1 + T-CMB-1 (blessings = bought conditions). Spec §6b–e. |
