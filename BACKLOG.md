...
| T-THR-1 | **Speaker-attributed thread log** — a readable in-thread speech/thought convention so conversations (NPC↔PC, OC↔OC, multiple models of one PC) are legible about *who says what to whom* | 🔥 engine | `in-progress` | frontdoor · sess:ceba401d-bd17-42e8-ba28-878c120b9d89 | 2026-07-20 | 🔥 LANE HELD. Building from plan `docs/superpowers/plans/2026-07-20-speaker-attributed-thread-log.md` (4 tasks). Full-identity treatment (bold+name+accent). Extends `threadView` render + model-overview; `speechColor` in `S`. |
...
| T-GX-G6 | **Wire G0 trait mechanics** — cross-Rift supply penalty, home-turf ruling trait, arrival & garrison scaling | canon + 🔥 engine | `blocked` | — | — | Blocked on G5 (Ultima) + 🔥 engine lane. **📋 Slice-1 plan ready:** `docs/superpowers/plans/2026-07-20-gx-g6-rift-modifier-core.md` (RIFT core + production flywheel, TDD, shovel-ready). §4.2/§4.3 = follow-on slices. |
...
| T-GX-G5 | Author **Ultima** segmentum | canon + tests | `in-progress` | galaxy · sess:406f1b0a-8f6e-4674-95ec-6d43178f11bd | 2026-07-20 | 🔒 CANON LANE HELD. The FINALE — the Front (mixed rift tags, a truly contested Scar) + seats the last 3 neutrals (Necrons/GSC/Harlequins) → completes ALL 20 factions, ~100 planets. Enriched sectors. Releasing fast. |
