...
|----|------|------|--------|-----------------|---------|-------|
| T-FD1 | **Front Door + persistence backbone** — title/start screen (`CONTINUE`/`NEW COMMANDER`/`TRY DEMO`/`SETTINGS`), **single active profile** save = full-`S` serialize/hydrate behind a **storage-adapter seam** (`LocalStore` now → `RemoteStore` drop-in later), Settings screen (AI · save mgmt · export/import · about), and wire the built-but-dead **Founding rite** → `commitFounding(cc)`. Unblocks live accumulation the living-world tick + galaxy both defer as "needs Stage-2 persistence". | 🔥 engine + tests + docs | `claimed` | frontdoor · sess:ceba401d-bd17-42e8-ba28-878c120b9d89 | 2026-07-19 | Epic. Pure SAVE core is tests-lane; 🔥 held only during boot-reseq/UI build. **Spec first (docs, parallel); flips to `in-progress` when I seize `index.html`.** Boot reseq preserves shipped `WORLD.catchUp`→digest order. |
