...
| T-TERR-1 | **Territory persistence + per-sector production (THE FLYWHEEL)** — save-state ownership of sectors/worlds so the living-world tick accrues by HOLDINGS, not a flat demo constant | 🔥 engine + canon | `in-progress` | terr · sess:f99107b3-ec8f-5a82-a770-4be40308d096 | 2026-07-21 | **P1 — scope EXPANDED per Daak:** territory flips must ride real thread resolution, so this task now ships (A) `THREAD.outcome`+conclusion (annihilation/mutual/accord → `t.done`, RESOLUTION post, force release), (B) `S.world.rulers` overlay + `S.world.holdings` (crown-world seed) + INVASION-victory capture, (C) `WORLD` production per-holding (`resources × prod_mult × RIFT home prodMult ÷ holding_divisor` → currency) **+ prosperity/conflict/taint axes** per `scores` spec, digest itemized. Personal holdings feed income (Daak call). Canon → v1.19. |
