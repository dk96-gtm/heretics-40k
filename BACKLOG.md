...
| T-ECO-1 | **Trade-run economy — distance/rarity arbitrage (NEEDS DEFINITION)** — buy at a sub-faction's own shop door, haul the goods to another segmentum, sell them to a *different* sub-faction at a premium: faction gear is cheap at home, rare and valuable far away | 🔥 engine + canon + docs | `open` | — | — | **Design pass first — Daak dictation 2026-07-21.** Questions to lock before any code: (1) **What makes an item "foreign"?** `it.faction` vs the buyer-door's ruling faction (`pRuler`) is the natural key — but shops currently only STOCK your-faction + common gear (`doorCatalog` filters by `S.player.faction`), so does stock become location-ruler-driven (shop sells the RULER's catalog, not yours)? That's the deeper, better change. (2) **Price curve** — premium by distance (same sector < cross-sector < cross-segmentum), by allegiance gap (Imperial↔Chaos pays more than Imperial↔Imperial), or by scarcity (has this item ever been sold here)? (3) **Anti-exploit** — must coexist with the shipped no-arbitrage rules: sell stays 60% of base at HOME; the premium only beats list price after real travel cost + Rift away-penalty (buy home ×1.0, sell away needs >1.67× to profit — tune so a cross-segmentum run nets ~20-40%). (4) **Provenance** — does gear need an `origin` stamp in `S.inv` (partially exists via cart `origin`) so the sell door knows how far it travelled? (5) Interaction with Comms **unsafe trade** + future escrow (T-ENG-2) — player↔player runs are the Stage-2 payoff. Output: a design doc in `docs/superpowers/specs/` + locked canon shape (`rules.trade`), THEN an implementation slice. |
