...
| T-NPC-2b | **NPC combat turn** — enemy AI picks moves+attacks via `reachable`/`spottedEnemies`/`bandOf`, stages a block, posts through `threadView`. Makes the enemy fight back. | 🔥 engine | `in-progress` | npcturn · sess:970009ad-d34c-4f64-9a2d-cd52fc53aac8 | 2026-07-20 | 🔥 LANE HELD. P1 — the solo-play jump. Pure decision fn in THREAD core (TDD) + thin threadView wiring; reuse NPCAI behavior axes. No new engine surface. |
| T-THR-1 | **Speaker-attributed thread log** — a readable in-thread speech/thought convention so conversations (NPC↔PC, OC↔OC, multiple models of one PC) are legible about *who says what to whom* | 🔥 engine | `claimed` | frontdoor · sess:ceba401d-bd17-42e8-ba28-878c120b9d89 | 2026-07-20 | Claimed. 🔥 lane currently held by T-NPC-2b → I spec/plan (docs, parallel) first, seize engine when free. Extends `threadView` render only; per-model `speechColor` in `S`. Spec = the **Log formatting convention** block below. |
...
| T-GX-G4 | Author **Tempestus** segmentum | canon + tests | `in-progress` | galaxy · sess:406f1b0a-8f6e-4674-95ec-6d43178f11bd | 2026-07-20 | 🔒 CANON LANE HELD. Nihilus heartland — seats Emperor's Children (completes all 8 Nihilus) + Daemons + Drukhari◇ + Tyranids. Rebuilds sealed stubs bakka/veil + 3 new sectors. Following G1/G3 contract. Releasing fast. |
