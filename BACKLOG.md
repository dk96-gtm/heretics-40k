...
| T-CMB-1 | **Combat tag-conditions tick & apply** — instance-based conditions (DoT/Regen/Slowing/Suppressing/…) tick, gate, and expire in the THREAD core | 🔥 engine + tests | `open` | — | 2026-07-25 | **DESIGN LOCKED 2026-07-25** — spec `docs/superpowers/specs/2026-07-25-combat-conditions-design.md`. Re-scoped: COMBAT TAGS ONLY (the 9 `galaxy.conditions` + `status_effects` are location/sector state → moved to T-STAT-1). Locked: afflicted-side tick anchor, FIFO tick order, hard validate gates, no stacking (refresh-or-upgrade), Suppressing capped at −1 action (tier = pin duration), extensible registry (unknown tags inert-but-displayed). Phase 2 = Shield/Ward/Decoy damage-step trio. Fixes the cond-label/self-target staging bug in passing. |
