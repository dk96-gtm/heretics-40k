...
| T-RIFT-1b | **Rift supply penalties — felt hooks (G6 slice 1b)** — apply the shipped `RIFT` core at existing seams so the Rift is felt in play | 🔥 engine | `in-progress` | terr · sess:f99107b3-ec8f-5a82-a770-4be40308d096 | 2026-07-21 | Building on the T-TERR-1 stack: pure `RIFT.applyTravel/applyReq` (TDD), `riftStandingAt` (overlay-aware sector owner via `secOwner`), travel passage ×travelMult at destination (preview + charge + route marker), requisition prices ×reqMult (shop/altar/reliquary display+cart, armoury buy/fit). Sell price stays base. |
