...
| T-RIFT-1b | **Rift supply penalties — felt hooks (G6 slice 1b)** — apply the shipped `RIFT` core at existing seams so the Rift is felt in play | 🔥 engine | `in-progress` | opus · sess:6e18d3b6-86ef-44ae-a594-6561cd9987e8 | 2026-07-21 | **Shovel-ready — pure core DONE (v1.18).** Travel: `passageCost` × `RIFT.mods(standing).travelMult` (home −25% / away +25%). Requisition: `doorCatalog`/shop pricing × `reqMult` (away +25%). Build force via `RIFT.forceOf(S.player.faction, D)`; standing from dest location `rift` + sector `owner` (`fPl`/`fLoc`). Thin glue at 2 sites; TDD each multiplier. See `docs/superpowers/plans/2026-07-20-gx-g6-rift-modifier-core.md` §follow-on. |
