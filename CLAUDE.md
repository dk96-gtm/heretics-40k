...
**Current state:** engine **v18** (+ flywheel/territory + capture/remains + missions + combat conditions), canon **data v1.27** — deployed and running. The full galaxy is minted (v1.12–v1.16): **5 segmentums · 27 sectors · 87 planets · 287 walkable locations · all 20 factions homed**.
...
- **v1.7 — Free-form slots + per-element armour:** `rules.loadout` (universal player-typed slots — no fixed weapon/item/ability split), `rules.growth.slot_gains_by_rank` (numeric slot growth), `rules.armour` (per-element Defense; a hit deals `max(0, dmg − def[element])`, corrosive-bypass expressed as data), a galaxy-wide `armour` catalog (143 pieces: 20 factions × class defaults + Light/Med/Heavy ladders + universal baseline), a 12th **Armoury** door (door retired by T-DOOR-2, 2026-08-02 — armour now sells via Shop), and a Forge `armour_upgrade` path (+1 Defense/element per tier I–III).
...
- **v1.11 — Living-world tick (NPC Slice 2a):** a `tick` block (`cadence:"day"`, `day_minutes:240` = 4 real hrs, `max_catchup_days:30`, flat `production_per_day`/`taint_per_day` demo rates). Anchors the deterministic real-time world-tick — the galaxy advances on wall-clock even when nobody's logged in, realized in Stage-1 as a lazy catch-up on load. Rates bind to real holdings when the territory migration (slices 4–6) lands. Design: `docs/superpowers/specs/2026-07-19-living-world-time-architecture-design.md`.
...
- **Requisition:** all 11 door types functional (shop/altar/reliquary catalogs + cart + sell; forge; muster; apothecarion; shipyard; relay; arena; warp gate; throne room). Location gating + closed-door reasons. Door demo mode. **Armoury door retired** (T-DOOR-2, canon v1.24, Daak ruling 2026-08-02): armour stock now sells through the Shop, weight-gated by the Shop's own tier (Heavy buyable at any Tier-III Shop); the Forge remains the armour-upgrade (hardening) path; fitting stays in the Barracks Armoury tab.
- **Free-form loadout + armour (v17):** every model's slots are universal — the player assigns each slot a type (Weapon/Item/Ability/Warp Cast) then equips into it; slot count derives from `base sl + rank growth`; edits gated to non-active models. Every model also has one hard **Armour slot** pre-filled with a class+faction default; armour carries a per-element Defense profile that `THREAD.apply` subtracts from incoming damage (`max(0, dmg − def[element])`, floored). Buy armour at a Shop (weight-gated by shop tier); harden a chosen element at the Forge. Pure `LOADOUT` core (`slotCount`/`legalItems`/`canEquip`/`retypeSlot`/`mitigate`) is node-tested.
- **Living-world tick + Digest (NPC Slice 2a):** a pure DOM-free `WORLD` core (`/*<world-core>*/`) runs a deterministic day-tick — `catchUp(state,canon,nowMs)` advances production + taint per elapsed in-game day, bounded by `max_catchup_days` (excess compressed), idempotent, `Date.now`-free (replayable). Boot fires the lazy catch-up on the persisted `S.time.lastTick`; a **World Digest** panel ("Since you last looked…") shows the templated summary, dismissing into a persistent **World Log** in Comms. NPC agency + sieges (2b) and thread response timers (2c) are the next sub-slices. Node-tested (`tests/world-core.test.js`).
...
- **Battlefield grid (v18):** combat threads run on a procedurally-generated square grid (`genBoard` from planet⊕location + RNG). Each combatant carries an `(x,y)` position; the abstract `band` scalar is gone — Chebyshev distance now *derives* the weapon range band (0-1 melee / 2-3 short / 4-6 medium / 7+ long). Speed is a free per-turn move radius (cost-based flood-fill: difficult terrain costs 2, impassable blocks, diagonals count 1); AP buys actions only. **True per-side fog of war** — every post renders the poster's knowledge (own models, spotted enemies, last-known `◌` ghosts, never-seen enemies absent); sight = equipped weapon band + scout/aspect bonus; `validate` gates attacks to currently-visible targets. Terrain (forest/ruins/mountain/fortification) blocks movement, blocks LOS when opaque, and grants damage-reducing cover. Combat threads open in a **blind deploy** phase (set your line in your zone, enemy deploys hidden) → **Lock In** freezes loadout + force roster and starts the battle. The board **is** the Action Block: click a model → reachable squares light → click to stage a move; click a spotted enemy → target → an in-range weapon button stages an attack; everything feeds the existing post → `validate` → `apply` pipeline (fog gate, AP pool, armour mitigation, wounds/kills). Pure grid helpers (`cheb`/`bandOf`/`reachable`/`lineOfSight`/`coverMod`/`genBoard`/`sightOf`/`spottedEnemies`/`refreshFog` + `apply` `move` effect) live in the DOM-free THREAD core and are node-tested (`tests/grid-*.test.js`). A standalone screen-VIII prototype (fixtures) also exists as a sandbox.
- **Combat conditions (T-CMB-1, canon v1.22):** combat tag-conditions are real. A pure `CONDS` registry in the THREAD core (10 tags: DoT/Regen/Slowing/Suppressing/Charging/Rally/Marked/Draining/Cleanse/Immunity + derived Injured/Critical) drives instance objects `{tag,tier,left,src,el,nl,nr,by}` on `combatants[].conds`; `tickConds` runs FIFO at the top of the afflicted side's post (frozen in deploy/aftermath); `validate` hard-gates condition-reduced speed/actions (fan-outs count once, cond-kind-gated); `applyCond` enforces Immunity-first + refresh-or-upgrade (no stacking, ever); staging builds real parsed-tag payloads with an ally picker + Rally/Cleanse fan-outs; damage effects carry `band` (Charging's melee bonus is live); `npcTurn` obeys its own pins. Daak rulings 2026-07-28: Non-Lethal sources floor DoT ticks at 1 wound (captureable); DoT kills carry full mission/weapon credit + no_revival permadeath. Legacy string conds self-heal via `normCond` (incl. JSON'd-Infinity repair). Phase 2 (Shield/Ward/Decoy) = T-CMB-4; coverage sweep = T-CMB-2; validate cond-gating = T-CMB-3 (design-gated).
- **Missions (Slice A, canon v1.21):** MISSION threads are winnable and paid. Objective tracker lives in the THREAD core (`state.objective` = `{kind,target,progress,params,done}`; kinds count_kill/collect_item/restore; `evalObjective`); a pure `/*<mission-core>*/` region generates per-planet boards on the world tick (stateless day⊕planet mulberry32 seeds — chunk-independent replay; door/NPC/notice faces; `payoutOf` = family_base × clamped size × prod_mult); boards persist in `S.world.missions` (+`missionSeedBase`/`missionDay`, seeded in BOTH `foundingWorld()` and `init()` — new `S.world` keys must seed in both). Location panel lists posted missions; accept (cap 3, idle-force + garrison gates, target clamped to spawnable hostiles w/ reprice) spawns a real thread; conclude pays `S.cur`, applies world effects, clears the board row; thread board doubles as Mission Log with progress meters. 3 pilot missions (Purge/Item Request/Rebuild); 8 more universals = Slice B (T-MSN-1B), streaks+signatures = Slice C (T-MSN-1C). Includes the T-THR-5 fix: `THREAD.create` prefers persisted `t.state` — live thread state survives reload.
- **Missions Slice B (T-MSN-1B, canon v1.27):** the remaining 8 universal rows ship — Bounty Hunt/Kill-Team/Assassination/Liberation/Defend (needs_hostiles, combat-kind incl. `survive_rounds`) + Trade Haul/Consecration/Desecration (allegiance-gated `gates.allegiance`). `state.round` (`THREAD.tickRound`, one tick per landed enemy post) drives Defend's meter; named-target missions (Bounty Hunt/Assassination) seed a deterministic boss (highest-pc model ×1.5, `named_premium` 1.5 in `payoutOf`) + reduced escort from the same rng stream as the board roll, with a target-name fallback for stale pre-1B rows; Liberation clears every hostile honestly (`clear_all`); a modifier picker (Understrength/Lone Wolf/Low-Tech/Ironman/Blitz) stacks the payout ×1.5 per modifier still valid at conclude (`THREAD.modCheck`, fail-closed on unknown ids) via `MISSION.finalPayout`. Boards mint player-agnostic; allegiance gating (`MISSION.gateReason`) is enforced at both render (hides gated rows) and accept (defense in depth) — a chaos/imperial-gated row is simply absent from the other side's board, never a visible-but-refused row.
- **Ultimatum clocks & the world fights back (T-NPC-3 N1, canon v1.33):** combat threads carry initiator-set Ultimatum clocks — a Short/Standard/Long window picked inside canon bands (`rules.ultimatum.bands`: raid 4-8 / skirmish 8-16 / invasion 12-24 game days; engine SKIRMISH→raid scale, INVASION→invasion via the data-driven `scale_of` map). While a clock runs the target location reads **Besieged** (locConds overlay; one clock per location). Expiry → a seeded garrison-favored auto-battle in the pure `/*<agency-core>*/` `ULT` core: defense = location level × `garrison_pc_per_level` × condition `garrison_mult` (Fortified 1.25/Intact 1/Sacked .6/Ruined .3) ×1.25 defender bonus vs the committed force's PC; seed = `missionSeedBase⊕expiresDay⊕hold:lid⊕agg:faction` (chunk-independent — lapses resolve tick-interleaved before that day's heal pass); outcome ladder repelled / repelled-w-losses (−1 condition step) / sacked (−1 step + seeded loot) / captured (INVASION-only planet transfer via `captureOnVictory`), arithmetic shown verbatim in the RECORD post, Digest and World Log. Damaged ground **self-heals** on the world tick, paced by sector status (Thriving 4 / Peace 6 / Corrupted 9 days per step; never in Warring/Famine; paused while Besieged; heals to Intact, never Fortified). Rebuild missions now truly repair (+1 step, `world_effect.repair_step`), and own damaged holdings offer manual no-payout **Rebuild/Liberate** actions in the location panel. **TRIBUTE** is the third ultimatum answer (fight/ignore/pay): the defender opens with a seeded offer (currency/resources/return of a TAKEN model), the attacker accepts / declines / counter-demands (×1.5/×2) — counters evaluated by NPC pragmatism + per-faction `tribute_appetite` (all 20 authored, flagged-for-review defaults), everything phase-gated to pre-Lock-In. Node-tested (`tests/agency-core.test.js`, `tests/canon-ultimatum.test.js`). N2 (NPC aggressor cadence/targeting) and N3 (Strategium screen) remain.
- **Personality combat doctrine (T-NPC-4, canon v1.34):** every seeded NPC fights in character. A 20-faction `ai.behavior_matrix` + `rules.doctrine` canon block backs a pure `AXES.rollFor`/`doctrineOf`/`shouldRetreat` region in the THREAD core — a deterministic per-thread roll (`doctrine:<threadId>` seed, faction-weighted, lazily stamped the first time the NPC side acts, so reloads replay identically) picks each NPC force's five combat axes (ferocity/cunning/pragmatism/honor/supremacism). The dominant axis sets a **style** that reshapes `npcTurn` targeting/movement: **ONSLAUGHT** (ferocity) closes on the nearest visible enemy; **CULLING** (cunning) targets the lowest-wounds visible enemy and kites at range once adjacent; **DECAPITATION** (supremacism) targets the highest-PC visible enemy (the "leader hunt"). **Honor conduct** reshapes the candidate pool independent of style: ≥70 seeks a duel (the NPC commander targets the enemy leader) and spares Critical (≤1 wound) targets; ≤30 finishes wounded targets first. **Pragmatism** drives withdrawal — `shouldRetreat` (retreat threshold `(110 − pragmatism)/100` of the force's PC lost) routes through the same minted Exit-escape speed check already used for player flight, so a fleeing NPC force either breaks contact (faster) or gets cornered and fights on, resolved once per thread (`state.retreatTried`) and never on duel/mission threads. Ultimatum lapse-battle rolls (T-NPC-3 N1) print personality nudges (cunning/ferocity/honor deltas) directly in the RECORD arithmetic. **Doctrine is fully hidden** — no style/axis label appears anywhere in the UI; behavior is legible only through what the NPC actually does on the board. Node-tested (`tests/canon-doctrine.test.js`, `tests/doctrine-core.test.js`, plus extensions to `tests/agency-core.test.js`). Tunables flagged for Daak review: the retreat curve constant (110), the honor gates (70/high, 30/low), the lapse nudge sizes, and all 100 authored `behavior_matrix` axis values.
