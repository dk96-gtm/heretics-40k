...
- **Seats, Trust & Standing (T-TERR-2, canon v1.34):** commanders can hold named locations on planets ruled by their own sub-faction. A pure `SEAT` core (`/*<seat-core>*/`) drives it: a 20×20 authored per-sub-faction **STANDING** ledger (seeded relations on founding, a 6-rung ladder WAR(-3)→ALLIED(+2); raiding any ruled world floors standing with its ruler to WAR) gates two rights — **petition** (own-kin + NEUTRAL standing + a per-location **WORK** trust counter earned by missions/rebuilds/manual service) buys a seat at `base×level×condMult`, and **planetary BUYOUT** (majority of a planet's seats + a sector-wide WORK floor (`rules.seats.buyout.sector_work_min`:20) + WARM standing) grants the whole planet at a `×2` premium over its unheld seats, stamping `S.world.governor[pid]=true`. A held seat pays a live **tax** (`3×level×condMult`/day, `seatTickIncome`) plus its typed production share (guarded against double-pay once the planet is a full `holdings` entry), can **station** one force (upkeep = all currency, non-payment wounds the *location* via `seatMiss`, never the force), commissions missing doors, and is Ultimatum-able. Stationed casualties on a seat's lapse ride the same N1 outcome ladder (15/35/60/100% pools, seeded, half carried off) and a seat falls automatically with its planet (capture frees any stationed force — no orphaned lock). Every ruled planet renders a **deterministic ruler NPC face** (`rulerFaceOf`, seeded off `missionSeedBase⊕planet id⊕faction` — title+name pool by allegiance, identical across reloads) in the Throne Room. Node-tested (`tests/seat-core.test.js`, `tests/canon-seats.test.js`). Tunables (matrix cells, seat bases/work gates, casualty pools, buyout premium/sector_work_min, upkeep divisor, ruler-face pools) are flagged-for-review defaults, not final balance.
