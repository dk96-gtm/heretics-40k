...
**Remember:** NPCs at a location get its recent chronicle records injected into their context (approach-in-person and comms lines) via the existing NPCAI tiered-memory seam — and the injection draws on the record's FULL reconstructed account (sides, closeness, what broke), not the headline. The record is a compressed full story, not a summary: nothing about the battle is forgotten, its prose is just not duplicated on disk while it remains a pure function of `record + seed`.

**AI seam (Daak ruling 2026-08-18 — fits the house AI pattern, same as thread summaries):** the record IS the context package for Stage-3 AI. Stage 1 spins the account through the templated narrator (deterministic, offline). At Stage 3, the same record becomes the AI prompt ("here are the battle logs — write the history") through the locked CF-proxy path, and the generated prose is **cached back onto the record** (`account` field, write-once) — AI text is not reproducible from a seed and costs quota, so once written it becomes the canonical account every reader and every NPC quotes. The ledger literally accumulates written history at Stage 3; Stage 1 needs no such field.
